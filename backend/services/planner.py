...
class QueryPlanner:
    ...
    def create_plan(self, user_input: RecommendationRequest) -> ScrapePlan:
        logger.info("QueryPlanner.create_plan called")
        try:
            ...
            logger.info(
                "QueryPlanner.create_plan allowed_targets=%s sections=%s",
                len(allowed_targets),
                len(domain_sections),
            )
            ...
            logger.info("QueryPlanner.create_plan llm_targets=%s", len(llm_targets))
            if llm_targets:
                allowed_by_host = {
                    target.url.host.removeprefix("www."): target
                ...
                logger.info(
                    "QueryPlanner.create_plan filtered_llm_targets=%s",
            ...
            logger.exception("QueryPlanner.create_plan failed: %s", exc)
            raise

    def _read_website_sections(self) -> dict[str, list[str]]:
        logger.info("QueryPlanner._read_website_sections called file=%s", self.websites_file)
    ...
    def _source_type_for(self, section: str, domain: str) -> str:
        logger.info("QueryPlanner._source_type_for called section=%s domain=%s", section, domain)
    ...
    def _normalize_url(self, domain_or_url: str) -> str:
        logger.info("QueryPlanner._normalize_url called value=%s", domain_or_url)
    ...
    def _goal_for(self, source_type: str, user_input: RecommendationRequest) -> str:
        logger.info("QueryPlanner._goal_for called source_type=%s", source_type)
